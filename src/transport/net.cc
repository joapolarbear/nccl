...
ncclResult_t netSendSetup(struct ncclTopoSystem* topo, struct ncclTopoGraph* graph, struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo, struct ncclConnect* connectInfo, struct ncclConnector* send, int buffSize, int channelId) {
  ...
  BPF_INFO_DUMP(NCCL_INIT|NCCL_NET,"Ring %02d : %d[%lx] -> %d[%lx] [send] via NET/%s/%d%s", channelId, myInfo->rank, myInfo->busId, peerInfo->rank, peerInfo->busId, ncclNetName(), resources->netDev,
...
ncclResult_t netRecvSetup(struct ncclTopoSystem* topo, struct ncclTopoGraph* graph, struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo, struct ncclConnect* connectInfo, struct ncclConnector* recv, int buffSize, int channelId) {
  ...
  BPF_INFO_DUMP(NCCL_INIT|NCCL_NET,"Ring %02d : %d[%lx] -> %d[%lx] [receive] via NET/%s/%d%s", channelId, peerInfo->rank, peerInfo->busId, myInfo->rank, myInfo->busId, ncclNetName(), resources->netDev,
      resources->useGdr ? "/GDRDMA" : "");
...
ncclResult_t netSendProxy(struct ncclProxyArgs* args) {
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    if (args->head < args->end) {
      ...
      if (args->head < args->tail) {
        ...
        if (done) {
          ...
          BPF_TIMELINE(args->unique_name, args->connector->comm->rank, args->connector->comm->cudaDev, false, start_t, sliceInfo, false);
    ...
    if (args->head == args->end) {
      ...
      ncclSliceInfo *sliceInfo = (ncclSliceInfo *)malloc(sizeof(ncclSliceInfo));
      netParseSliceId(args, sliceInfo);
      BPF_TIMELINE(args->unique_name, args->connector->comm->rank, args->connector->comm->cudaDev, true, 0, sliceInfo, false);
    }
  }
  return ncclSuccess;
}
